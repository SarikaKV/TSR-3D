...
import math,numpy,ntpath,glob, csv, os
...
class FeatureSelection:
    ...
    def generate_dict(self):
        for i in self.filesList:
            ...
            f1.close()
        with open(os.path.join(self.outfolder, 'keys_frequency_list.csv'), 'w') as csv_file:
            writer = csv.writer(csv_file)
            for key, value in self.allKeyCountDict.items():
    ...
    def feature_selection(self):
        ...
        for keys_ in loopThruDict:
            list_=[]
            list_=list(map(float,loopThruDict[keys_]))
