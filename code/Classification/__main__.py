...
parser.add_argument('--featureSelection_csr', '-featureSelection_csr', action='store_false', default=False, \
	help='Argument to tell if Feature Selection in CSR format required')
...
parser.add_argument('--add_header', '-add_header', action='store_true', default=True, \
    help='Enable this option if headers are required for feature map in step 3.')
...
def run_classification_pipeline(**kwargs):
	...
	setting = '_' + kwargs['outFolderName'] + '_gap' + str(kwargs['numGap']) + \
			  ...
			  if kwargs['featureSelection'] else '_' + outFolderName + '_NoFeatureSelection' \
			  	+ '_keyCombine' + str(kwargs['keyCombine'])
	print(setting)
	...
	if 1 in steps:
		...
		keys = KeyGeneration(path = kwargs["path"],subFolder = kwargs["subFolder"],\
			fileType = kwargs["filesType"] ,aminoAcidCode = AMINO_ACID_CODE, \
			thetaBounds = THETA_BOUNDS, distBounds = DIST_BOUNDS, numOfLabels = NUM_LABELS)
		Parallel(n_jobs=cpu_count() - 1, verbose=10, backend="multiprocessing", batch_size="auto")(\
			delayed(keys.processFiles)(fileName) for fileName in files)
	...
	files=glob.glob(kwargs["outFolder"]+'//*.keys*')	
	...
	# # Feature Selection
	if kwargs['featureSelection_csr']:
		if (2 in steps) or (3 in steps):
			print('--------------------------Start Vectorization NO FS-----------------------------------')
			if kwargs['keyCombine'] == 0:
				changedFiles=glob.glob(kwargs["outFolder"]+'//*.keys*')
			else:
			...
			vectors = VectorizationNoFS(outFolder = outFolder, setting = setting, filesList = changedList)
			vectors.vectorize()
			print('--------------------------End Vectorization NO FS-------------------------------------')
	else:
		if 2 in steps:	
		...
		if 3 in steps:
			...
			vectors = Vectorization(outFolder = outFolder, setting = setting, filesList = changedList, \
				details_dict = df_dict, add_header= kwargs["add_header"])
			vectors.vectorize()
			print('--------------------------End Vectorization-------------------------------------')
		# MDS
		if 7 in steps:
			print('--------------------------Start PCA--------------------------------------')
			pca = VectorizationFSPCA(outFolder = outFolder, setting = setting, sample_dict = df_dict)
	...
	if 5 in steps:
		...
		visualize = Visualize(samplesFile = df, outFolder = outFolder, setting = setting, \
			details_dict = df_dict, has_columns = True)
	...
	if 6 in steps:
		print('--------------------------Start MDS--------------------------------------')
		mds = MDS(samplesFile = df, outFolder = outFolder, setting = setting, has_columns = False)
...
if __name__ == '__main__':
	...
	amino_acid_code=open(os.path.join(args.path, "aminoAcidCode_map.txt"),"r") 
	...
	run_classification_pipeline(path = args.path, subFolder= subFolder, thetaBounds = thetaBounds, \
		outFolderName= outFolderName, sample_name=args.sample_name, featureSelection_csr = args.featureSelection_csr,\
		...
		keyCombine = args.keyCombine, numOfLabels= args.numOfLabels, add_header= args.add_header,\
